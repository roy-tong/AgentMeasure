...
- **DR-005 revision — two states + ranked evidence**: adopted the client-side reviewer's reframing (direct reply 2026-08-24): `referenced` is not a semantic state but a weak estimator for influence with two named failure modes (cite-without-use, use-without-cite); the spec keeps exactly two states — availability (context fact) and influence (behavioral fact, experiment layer). Added a third reviewer's provider inference boundary (direct reply 2026-08-24, independent convergence): client-side telemetry certifies `serialized-as-sent`, one step short of `reached-inference`. Ladder diagram replaced with the two-state/evidence-rank form. Issue #11 body and normative ontology updated to match; reviewer attribution across DR-005/DR-006 switched to anonymous-pending-consent.
