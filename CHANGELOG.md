...
- **M3 execution-grain conformance vectors** ([metric-execution-grain](conformance/vectors/metric-execution-grain.json), runner in `run_metrics.py`): operationalizes DR-006 (emission grain ≠ analysis grain) with three vectors — a runtime retry chain (429 → timeout → success under one `operation_id`) resolving to M3.1 = 1 operation with M3.3 = 0.333 attempt success and `attempts_per_operation` = 3 disclosed; three independent executions with no grouping evidence staying unresolved (M3.1 = 0, fail-closed, `legacy_attempt_equivalent` disclosed instead of disguised); and the same three attempts read at both grains (the "33% vs one logical use" disclosure case). External evidence: Avi Seth 2026-08-24 (operation as additive grouping key, not the emitted unit); Arthi (append-only attempt facts).
- **DR-005 revision — two states + ranked evidence**: adopted Gunjan Jaswal's reframing (2026-08-24 reply): `referenced` is not a semantic state but a weak estimator for influence with two named failure modes (cite-without-use, use-without-cite); the spec keeps exactly two states — availability (context fact) and influence (behavioral fact, experiment layer). Added David Elsey's provider inference boundary (2026-08-24, independent convergence): client-side telemetry certifies `serialized-as-sent`, one step short of `reached-inference`. Ladder diagram replaced with the two-state/evidence-rank form.
