# agent-used-claude — Claude Code Adapter（设计）

> Claude Code 原生支持 OTLP 输出（metrics / events / traces），官方文档明确工具调用频率、
> 成功率、耗时与 MCP activity 作为 telemetry 场景。
> 原则：**不要求用户放弃现有 observability backend**——agent-used 作为 OTel Processor/Exporter。
...
Claude Code
      ↓ OTLP
agent-used Collector（本地）
        ├─→ 用户自己的 Grafana / Langfuse / 现有 backend（原样转发）
        └─→ privacy-safe aggregates → agent-used 公开统计
```
...
| 步骤 | 内容 | 状态 |
| --- | --- | --- |
| P1 | agent-used Collector 配置模板（OTLP 接收 → 分流 → 本地聚合） | 待开发 |
...
- [ ] Claude Code 配置 agent-used collector 后，工具调用产生本地 usage 记录
