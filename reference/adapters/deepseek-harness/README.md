...
> DSH 是 AgentMeasure 的第一方深集成目标（cross-side corroboration 证据能力的来源）。
...
关联机制：`tool/result` 通过 `sourceEventSeqs: [callSeq]` 引用其 `tool/call` 事件——**DSH 原生提供配对关系**（agent-side 的 corroboration 半边）。
...
- `tool/call ↔ tool/result` 的 sourceEventSeqs 配对是 harness 原生证明；**证据等级由 verifier 派生（本 adapter 绝不自声明）**——verifier 可据此判 corroborated
- 若工具侧（MCP wrapper）同 trace_id 关联，构成真正的 cross-side corroborated（跨 harness 与工具）
...
| P3 | sourceEventSeqs 配对 → verifier 派生 corroboration | 已实现（completed lifecycle 配对） |
...
- [ ] 工具侧接入时升级为 cross-side corroborated
